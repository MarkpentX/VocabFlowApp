        ...
        <main className="flex flex-col gap-3 px-6 py-4 mt-1.5 max-w-5xl h-dvh mx-auto">
            <div className="items-start justify-start">
                ...
                <div className="mt-6 grid gap-x-3 gap-y-4 ">
                    {words.map((item) =>(
                        <article className="flex gap-2 bg-[rgb(255,255,255)] items-center border-[rgb(226,229,220)] drop-shadow-sm py-6 shadow-black p-4 rounded-xl justify-between" key={item.id}>
                            <div className="flex items-center gap-3 min-w-0">
                                <SoundButton word={item.infinitive} lang="en" className="mx-3 shrink-0"/>
                                <h3 className="font-dMSans text-[rgb(18,33,28)] truncate">{item.infinitive}</h3>
                                <h4 className="font-dMSans text-[rgb(103,126,119)] truncate">- {item.meaning}</h4>
                            </div>
